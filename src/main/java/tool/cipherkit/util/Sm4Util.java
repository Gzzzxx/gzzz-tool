package tool.cipherkit.util;

import tool.cipherkit.common.constant.CommonConstant;
import tool.cipherkit.common.enums.AlgEnums;
import tool.cipherkit.common.enums.IEnum;
import tool.cipherkit.web.domain.request.Sm4Request;
import tool.cipherkit.web.domain.response.Sm4Response;
import tool.cipherkit.web.exception.base.BusinessException;
...
public class Sm4Util {
    ...
    private static Cipher generateCipher(String algorithmName, String mode, byte[] key, byte[] iv, int opMode) throws Exception {
        ...
        try {
            if (CommonConstant.ECB.equals(mode)) {
                cipher.init(opMode, sm4Key);
            }
            if (CommonConstant.CBC.equals(mode) || CommonConstant.CTR.equals(mode)) {
    ...
    private static byte[] handleKey(Sm4Request request) {
        ...
        if (CommonConstant.BASE64.equals(request.getKeyType())) {
            DecideUtil.decideBase64(request.getKey(), CommonConstant.KEY);
    ...
    private static byte[] handleData(Sm4Request request, String type) {
        ...
        if (CommonConstant.TEXT.equals(request.getDataType())) {
            ...
            if (DECRYPT.equals(type)) {
                DecideUtil.decideBase64(request.getData(), CommonConstant.DATA);
    ...
    private static byte[] handleIv(Sm4Request request) {
        ...
        if (CommonConstant.CBC.equals(request.getMode()) || CommonConstant.CTR.equals(request.getMode())) {
            ...
            if (CommonConstant.BASE64.equals(request.getKeyType())) {
                DecideUtil.decideBase64(request.getIv(), CommonConstant.IV);
    ...
    private static void checkParam(Sm4Request request) {
        ...
        if (CommonConstant.CBC.equals(request.getMode()) && StrUtil.isBlank(request.getIv())) {
            throw new BusinessException("CBC模式向量不能为空！");
        }

        if (CommonConstant.CTR.equals(request.getMode()) && StrUtil.isBlank(request.getIv())) {
